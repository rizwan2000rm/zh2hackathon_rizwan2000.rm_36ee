...
const SplitBillReq = ({ activeRequest, setActiveRequest, history }) => {
  ...
  return (
    <div className="max-w-sm mx-auto">
      <h1 className="text-xl font-medium text-gray-700">New Split Request</h1>
      <div className="rounded-lg px-1 border border-gray-300 py-4  mt-2 sm:px-1 sm:py-8 sm:mt-5">
        {activeRequest ? (
          <>
            ...
            <p className="text-center mx-auto px-8 text-sm pt-2">
              ...
              <span className="font-medium text-yellow-400">${mySplit}</span>{" "}
              from <span className="font-bold">{activeRequest.name}</span>
            </p>
          ...
          <div className="p-5 text-center"> No New Requests</div>
        )}
      </div>
    </div>
  );
